...
export default function MainMenuList() {
  ...
  const fetchData = async (overrideParams: any = {}) => {
    try {
      ...
      const res = await fetch(`/list_submenu?${query}`);
  ...
  const fetchMenuDetail = async (id: number) => {
    try {
      const res = await fetch(`/master_submenu/detail_submenu/${id}`);
      if (!res.ok) throw new Error('Gagal mengambil data');
      const json = await res.json();
  ...
  const handleCategoryChange = (field: string, selected: any) => {
    setFilters(prev => ({
      ...prev,
      [field]: selected && 'value' in selected ? selected.value : '',
    }));
  };
  ...
  const handleSaveOrUpdate = async () => {
    ...
    let url = '/master_submenu/store';
    let method: 'POST' | 'PUT' = 'POST';
    if (modalMode === 'edit' && editRow) {
      url = `/master_submenu/update/${editRow.msid}`;
      method = 'PUT';
    }
      ...
      <div className="bg-gray-100 min-h-screen p-6 space-y-6">
        ...
        >
          <form className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div>
              <label htmlFor="menuName">Nama Submenu</label>
              <input
                type="text"
                name="name"
